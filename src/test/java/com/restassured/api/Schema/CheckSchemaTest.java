package com.restassured.api.Schema;
...
public class CheckSchemaTest {
	...
	public void bookings_Schema_Test(){
		...
		given().log().all()
			...
			.body(new File("\Users\harika\workspace\NaveenRestfulApi\src\test\java\com\restassured\api\Resources\BookingsSchema.json"))
			.when().log().all()
				.post("/booking")
		.then().log().all()
			.assertThat()
